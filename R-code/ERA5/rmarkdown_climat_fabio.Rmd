...
The PET data is scattered across several files (one per year) so we use
`lapply` to read several files with a single function call. Here, we first
extract data for four sites of interest on which we want to focus.
